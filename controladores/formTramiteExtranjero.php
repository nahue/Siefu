                    ...
                    if (json === true)  {
                        ...
                        $.ajax({
                            ...
                            success: function(data){
                                ...
                                {
                                    //window.location.href = '<?= Html::urlModulo("generarPDF") ?>'
...
<form id="formTramite" name="form1" method="post" action="index.php?controlador=procesarFormulario">
    <h2>Registro Nacional de las Personas - Formulario Único</h2>
    <input type="hidden" name="extranjero" value="1" />
    <!-- SIEMPRE VA -->
...
if (!in_array("identificacion", $tramites)) {
    ...
    include("tramites/b3_educacional.php");
}

if (
        in_array("actualizacionMayor16", $tramites)
        ||
        in_array("actualizacionMenor", $tramites)
        ||
        in_array("identificacion", $tramites)
        ||
        in_array("prorroga", $tramites)
        ||
        in_array("cambioCategoria", $tramites)
)
    include("tramites/b4.php")
?>
